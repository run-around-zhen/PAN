...
def main():
    ...
    for k, v in map_key_to_id.items():
        ...
        ax.set_xlabel("流大小(B)", fontproperties= zhfont,fontsize=12.5)
        ...
        for tgt_lbmode in lbmode_order:
            for vv in v:
                ...
                if lb_mode == tgt_lbmode:
                    ...
                    elif config_id == "74025710":
                        ax.plot(xvals,
                        result["avg"],
                        markersize=1.0,
                        linewidth=3.0,
                        label="{}-two-path".format(lb_mode))
                    elif config_id == "991986616":
                        ax.plot(xvals,
                        result["avg"],
                        markersize=1.0,
                        linewidth=3.0,
                        label="{}-10".format(lb_mode))
                    elif config_id == "495130826":
                        ax.plot(xvals,
                        result["avg"],
                        markersize=1.0,
                        linewidth=3.0,
        ...
        ax.set_xlabel("流大小(B)",  fontproperties= zhfont, fontsize=12.5)
        ax.set_ylabel("尾流完成时间减缓",fontproperties= zhfont, fontsize=12.5)
        ...
        ax.set_xlabel("Time (ns)", fontsize=11.5)
        ...
        for tgt_lbmode in lbmode_order:
            for vv in v:
                ...
                if lb_mode == tgt_lbmode:
                    ...
                    if config_id == "358838415" or config_id == "625288335":
                        ax.plot(xvals,
                        result["avg"],
                        markersize=1.0,
                        linewidth=3.0,
                        label="{}-origin".format(lb_mode))
                    else:
                        ax.plot(xvals,
                        result["avg"],
                        markersize=1.0,
