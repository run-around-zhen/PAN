...
def main():
        ...
        for line in f.readlines():
            for topo in topo2bdp.keys():
                if topo in line:
                    ...
                    if lb_mode != "Conweave" and lb_mode != "Gemma" and lb_mode != "Conga" and lb_mode != "Letflow":
                        continue
                    if lb_mode == "Letflow":
                        lb_mode = "LetFlow"
                    if lb_mode == "Conga":
    ...
    for k, v in map_key_to_id.items():
        ...
        ax.set_xlabel("入口队列长度(KB)", fontproperties= zhfont, fontsize=12.5)
        ...
        for vv in v:
            ...
            with open(filename_voq_volume, "r") as f:
                for line in f.readlines():
                    parsed_line = line.replace("\n","").split(" ")
                    data_voq_volume["x"].append(float(parsed_line[0]) / 1024)
        ...
        fig_filename = fig_dir + "/{}.svg".format("CDF_INQUEUE_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
