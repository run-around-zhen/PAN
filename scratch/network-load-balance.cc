...
void periodic_monitoring(FILE *fout_voq, FILE *fout_voq_detail, FILE *fout_uplink, FILE *fout_conn,
    ...
    for (const auto &tor2If : torId2UplinkIf) {  // for each TOR switches
        ...
        // if (lb_mode_val == 3 || lb_mode_val == 6) {
        //     uint32_t ingressBuffer = 0;
        //     uint32_t egressBuffer = 0;
        //     for (int i = 0; i < 128; i++) {
        //         ingressBuffer += swNode->m_mmu->GetUsedBufferTotal();
        //         // egressBuffer += swNode->m_mmu->m_usedEgressTotalBytes;
        //     }
        //     fprintf(fout_voq, "%lu,%u,%u,%u\n", now, tor2If.first, ingressBuffer,
        //                 egressBuffer);
        // }
        // common: monitor TOR's uplink to measure load balancing performance
        
        for (int i = 1; i <= 16; i++) {
            uint64_t in_bytes = swNode->GetAllBytesInDev(i);
            fprintf(fout_uplink, "%lu,%u,%u,%lu,%lu\n", now, tor2If.first, i, in_bytes, in_bytes);
        }
    ...
    for (uint32_t i = 0; i < Settings::node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            auto swNode = DynamicCast<SwitchNode>(sw1);
             if (lb_mode_val == 3 || lb_mode_val == 6) {
                for (int j = 1; j <= 8; j++) {
                    fprintf(fout_voq, "%lu,%u,%u,%u\n", now, j, swNode->m_mmu->m_usedIngressPortBytes[j], i);
                }
            }
...
void conga_history_print() {
    ...
    std::cout << "Number of flowlet's timeout:" << CongaRouting::nFlowletTimeout << std::endl
              ...
              << "触发的pfc次数为:" << PfcWarning::m_pasuedNum << std::endl;
}
...
void letflow_history_print() {
    ...
    std::cout << "Number of flowlet's timeout:" << LetflowRouting::nFlowletTimeout
              << "\n数据包的数量为:" << LetflowRouting::nPacketNum
              << "\n遇到的pfc暂停的数量为:" << LetflowRouting::nPauseFollowNum
...
void conweave_history_print() {
    ...
    std::cout << "Number of INIT's Reply sent (RTT_REPLY):" << ConWeaveRouting::m_nReplyInitSent
              ...
              << "\nNumber of MaxRepeatNum:" << Settings::maxRepeatFlowNum << std::endl
              << "触发的pfc次数为:" << PfcWarning::m_pasuedNum << std::endl;
...
void gemma_history_print() {
    ...
    std::cout << "路径变化的次数为:" << GemmaRouting::m_RankChangeNum
              ...
              << "\n无效的重新路由数量为:" << GemmaRouting::m_unusedRerouted << std::endl
              << "触发的pfc次数为:" << PfcWarning::m_pasuedNum << std::endl;
...
void qp_finish(FILE *fout, Ptr<RdmaQueuePair> q) {
    ...
    // if (Settings::cnt_finished_flows % 1000 == 0 || Settings::cnt_finished_flows > 24000) {
...
int main(int argc, char *argv[]) {
    ...
    for (auto &pair : link_pairs) {
        ...
        if (probably_host->GetNodeType() == 0 && probably_switch->GetNodeType() == 1) {
            ...
            sw->m_isToR = true;
            sw->m_mmu->m_pfcWaring.m_isToR = true;
    ...
    if (lb_mode == 3 || lb_mode == 6 || lb_mode == 9 || lb_mode == 10) {  // Conga, Letflow, Conweave
                    ...
                    for (auto j = table1.begin(); j != table1.end(); j++) {
                        ...
                        for (auto next1 : nexts1) {
                            ...
                            if (nexts2.size() == 1 && nexts2[0]->GetId() == swDstId) {
                                ...
                                Settings::IdPortToId[GetSwitchPortKey(aggId, outPort2)] = swDstId; 
        ...
        for (auto i = nextHop.begin(); i != nextHop.end(); i++) {  // every node
            if (i->first->GetNodeType() == 1) {
                Ptr<Node> node = i->first;
                Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(node);  // switch
                sw->m_mmu->m_pfcWaring.m_switch_id = sw->GetId();
    ...
    if (lb_mode == 9 || lb_mode == 10 || lb_mode == 3 || lb_mode == 6) {
