...
void CongaRouting::RouteInput(Ptr<Packet> p, CustomHeader ch) {
    ...
    if (m_isToR) {     // ToR switch
        if (!found) {  // sender-side
            ...
            if (flowletItr != m_flowletTable.end()) {
                ...
                if (now - flowlet->_activeTime <= m_flowletTimeout) {  // no timeout
                    ...
                    if (Settings::m_pausedInfo[m_switch_id][outPort]) {
                        ...
                        // uint32_t newPath = GetBestNoPausePath(dstToRId, ch.udp.pg);
                        // if (newPath == 0) {
                        //     std::cout << "每一条路都被停了" << std::endl;
                        // }else {
                        //     if (GetPathCongestion(dstToRId, selectedPath) <= GetPathCongestion(dstToRId, newPath)) {
                        //         selectedPath = newPath;
                        //         flowlet->_PathId = selectedPath;
                        //         outPort = GetOutPortFromPath(selectedPath, 0);
                        //         // std::cout << "挑选一条更拥堵的路径" << std::endl;
                        //     }
                        // }
                    }
            ...
            uint32_t outPort = GetOutPortFromPath(selectedPath, 0);
            if (Settings::m_pausedInfo[m_switch_id][outPort]) {
                // std::cout << "挑选的路径被pfc暂停辣,挑选一条没有pfc暂停的路,这条路的分数为:" << GetPathCongestion(dstToRId, selectedPath) << std::endl;
                CongaRouting::nPauseFlowletNum++;
                // uint32_t newPath = GetBestNoPausePath(dstToRId, ch.udp.pg);
                // if (newPath == 0) {
                //     std::cout << "每一条路都被停了" << std::endl;
                ...
                //         selectedPath = newPath;
                //         outPort = GetOutPortFromPath(selectedPath, 0);
                //        //  std::cout << "挑选的路径不比原来差" << std::endl;
                //     }
                    
                //     // std::cout << "新挑选的路径分数为:" << GetPathCongestion(dstToRId, selectedPath) << std::endl;
                // }
            }
...
uint64_t CongaRouting::GetFlowKey(uint32_t ip1, uint32_t ip2, uint16_t port1, uint16_t port2) {
    /** IP_ADDRESS: 11.X.X.1 */
...
/* CongaRouting's main function */
void CongaRouting::RouteInputPfc(Ptr<Packet> p, CustomHeader ch,  PfcWarning pfcWaring) {
    // Packet arrival time
    Time now = Simulator::Now();
            ...
            if (flowletItr != m_flowletTable.end()) {
                ...
                if (now - flowlet->_activeTime <= m_flowletTimeout) {  // no timeout
                    ...
                    if ((pfcWaring.m_isWarning[outPort] || pfcWaring.SwitchPaused(m_switch_id, selectedPath)) && !m_isBigFlow[qpkey]) {
                        ...
                        uint32_t waringTime = Simulator::Now().GetNanoSeconds();
                        if (randNum < waringTime) {
                            uint32_t newPath = GetBestNoPausePath(dstToRId, ch.udp.pg, pfcWaring);
                            if (newPath == 0) {
                                std::cout << "每一条路都被停了" << std::endl;
                            }else {
                                selectedPath = newPath;
                ...
                // if (Settings::m_pausedInfo[m_switch_id][outPort]) {
                //     std::cout << qpkey << ":流超时,选择的路径被pfc暂停辣,直接换路,分数为:" << 
                //             GetPathCongestion(dstToRId, selectedPath) << std::endl;
                //     CongaRouting::nPauseFollowNum++;
                // }
                uint32_t X = UpdateLocalDre(p, ch, outPort);  // update
...
uint32_t CongaRouting::GetRandomPath(uint32_t dstToRId) {
...
uint32_t CongaRouting::GetBestNoPausePath(uint32_t dstToRId, uint32_t qIndex, PfcWarning pfcWaring) {
    // std::cout << m_switch_id << " " << dstToRId << " 开始找路" << std::endl; 
    auto pathItr = m_congaRoutingTable.find(dstToRId);
    assert(pathItr != m_congaRoutingTable.end() && "Cannot find dstToRId from ToLeafTable");
    std::set<uint32_t>::iterator innerPathItr = pathItr->second.begin();
    // std::advance(innerPathItr, rand() % pathItr->second.size());
    ...
    // path info for remote congestion, <pathId -> pathInfo>
    auto pathInfoMap = m_congaToLeafTable[dstToRId];
    std::map<uint32_t, uint32_t> congestionRank;
    std::vector<uint32_t> pathVec;

    // get min-max path
    std::vector<uint32_t> candidatePaths;
    uint32_t minCongestion = CONGA_NULL;
    uint32_t maxCongestion = 0;
    for (uint32_t i = 0; i < pathItr->second.size(); i++) {
        ...
        auto outPort = GetOutPortFromPath(pathId, 0);  // outPort from pathId (TxToR)
        if (pfcWaring.m_isWarning[outPort] || pfcWaring.SwitchPaused(m_switch_id, pathId)) {
            // std::cout << m_switch_id << " " << outPort << " 接收到预警" << std::endl;
            std::advance(innerPathItr, 1);
            continue;
        }
        // local congestion -> get Port Util and quantize it
        ...
        // get maximum of congestion (local, remote)
        uint32_t CurrCongestion = std::max(localCongestion, remoteCongestion);
        congestionRank[pathId] = CurrCongestion;
        ...
        maxCongestion = std::max(maxCongestion, CurrCongestion);
        std::advance(innerPathItr, 1);
    }
    for (int i = 0; i < pathVec.size(); i++) {
        uint32_t mId = i;
        for (int j = i+1; j < pathVec.size(); j++) {
            if (congestionRank[pathVec[j]] < congestionRank[pathVec[mId]]) {
                mId = j;
            }
        }
        std::swap(pathVec[i], pathVec[mId]);
    }
    
    // for (int i = 0; i < pathVec.size(); i++) {
    //     std::cout << GetOutPortFromPath(pathVec[i], 0) << " " << pathVec[i] << " " << congestionRank[pathVec[i]] << std::endl;
    // }
