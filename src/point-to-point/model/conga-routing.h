...
class CongaRouting : public Object {
   ...
   public:
    ...
    static uint64_t GetFlowKey(uint32_t ip1, uint32_t ip2, uint16_t port1, uint16_t port2);
    ...
    void RouteInputPfc(Ptr<Packet> p, CustomHeader ch, PfcWarning pfcWaring);
    ...
    uint32_t GetBestNoPausePath(uint32_t dstTorId, uint32_t qIndex, PfcWarning pfcWaring);
   ...
   private:
    ...
    std::map<uint32_t, bool> m_isBigFlow;
};
