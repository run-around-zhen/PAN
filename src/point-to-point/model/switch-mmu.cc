    ...
    if (m_dynamicth) {
        for (uint32_t i = 0; i < qCnt; i++) {
            ...
            if ((double)m_usedIngressPGBytes[port][i] - m_pg_min_cell - m_port_min_cell >
                ...
                m_usedIngressPGHeadroomBytes[port][qIndex] != 0) {
                pClasses[i] = true;
                // std::cout << "触发pfc,此时占用的入口队列为:" << m_usedIngressPortBytes[port] << std::endl;
            }
        }
    } else {
...
bool SwitchMmu::GetWarnMessage(uint32_t port, uint32_t qIndex) {
    // std::cout << "目前占用的量为:" << m_usedIngressPortBytes[port]  << std::endl;
    if (port > m_activePortCnt) {
        std::cerr << "ERROR: port is " << port << std::endl;
    }
    if (!m_isPortWarning[port]) {
        if (((double)m_usedIngressPGBytes[port][qIndex] - m_pg_min_cell - m_port_min_cell >
                    m_pg_shared_alpha_cell * ((double)m_buffer_cell_limit_sp -
                                              m_usedIngressSPBytes[GetIngressSP(port, qIndex)]) - m_pg_pfc_warn_diff * MTU) ||
                m_usedIngressPGHeadroomBytes[port][qIndex] != 0) {
            m_isPortWarning[port] = true;
            return true;
        }
    }  
    return false;
...
bool SwitchMmu::GetResumeMessage(uint32_t port, uint32_t qIndex) {
    if (!m_isPortWarning[port]) return false;
 
    if ((double)m_usedIngressPGBytes[port][qIndex] - m_pg_min_cell - m_port_min_cell <
                    m_pg_shared_alpha_cell * ((double)m_buffer_cell_limit_sp -
                                              m_usedIngressSPBytes[GetIngressSP(port, qIndex)]) - (m_pg_pfc_warn_diff+20) * MTU && 
        m_usedIngressPGHeadroomBytes[port][qIndex] == 0) {
        m_isPortWarning[port] = false;
        return true;
    }
    return false;
}
...
void SwitchMmu::SetPause(uint32_t port, uint32_t qIndex, uint32_t pause_time) {
    // std::cout << m_congaRouting.m_switch_id << "交换机端口号:" << port << "被暂停了" << std::endl;  
    if (paused[port][qIndex] == false) {
        PfcWarning::m_pasuedNum ++;
    }
    ...
    if (m_pfcWaring.m_isToR && port < 17) {
        return;
    }
    uint32_t originSwitchId = Settings::IdPortToId[m_pfcWaring.GetSwitchPortKey(m_pfcWaring.m_switch_id, port)];
    uint32_t originPort = 0;
    uint32_t begin = 17;
    if (m_pfcWaring.m_isToR) begin = 1;
    for (int i = begin; i < begin+8; i++) {
        if (Settings::IdPortToId[m_pfcWaring.GetSwitchPortKey(originSwitchId, i)] == m_pfcWaring.m_switch_id) {
            originPort = i;
            break;
        }
    }
    // if (originSwitchId >= 128 && !Settings::m_pausedInfo[originSwitchId][originPort]) {
    //     std::cout << originSwitchId << " " << originPort << " 被暂停了" << std::endl;
    // }
    
    Settings::m_pausedInfo[originSwitchId][originPort] = true;
}
void SwitchMmu::SetResume(uint32_t port, uint32_t qIndex) {
    // std::cout << m_congaRouting.m_switch_id << "交换机端口号:" << port << "恢复了" << std::endl;  
    ...
    // if (originSwitchId >= 128) {
    //     std::cout << originSwitchId << " " << originPort << " 恢复了" << std::endl;
    // }
    Settings::m_pausedInfo[originSwitchId][originPort] = false;
}
