...
class SwitchMmu : public Object {
   public:
    ...
    bool GetWarnMessage(uint32_t port, uint32_t qIndex);
    bool GetResumeClasses(uint32_t port, uint32_t qIndex);
    bool GetResumeMessage(uint32_t port, uint32_t qIndex);
    ...
    uint32_t m_usedEgressTotalBytes{0};
