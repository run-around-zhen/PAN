...
SwitchNode::SwitchNode() {
    ...
    m_mmu->m_pfcWaring.SetSwitchSendCallback(MakeCallback(&SwitchNode::DoSwitchSend, this));
    m_mmu->m_pfcWaring.SetSwitchSendToDevCallback(
        MakeCallback(&SwitchNode::SendToDevContinue, this));
...
uint32_t SwitchNode::DoLbLetflow(Ptr<Packet> p, CustomHeader &ch,
    ...
    //uint32_t outPort = m_mmu->m_letflowRouting.RouteInputPFc(p, ch, m_mmu->m_pfcWaring);
...
void SwitchNode::CheckAndSendPfc(uint32_t inDev, uint32_t qIndex) {
    ...
    // if (!m_isToR) {
    //     std::cout << "目前在中间交换机上" << std::endl;
    // }
    
    for (int j = 0; j < qCnt; j++) {
        if (pClasses[j]) {
            // std::cout << "交换机" << GetId() << " " << inDev << "超出阈值" << std::endl;
...
bool SwitchNode::SwitchReceiveFromDevice(Ptr<NetDevice> device, Ptr<Packet> packet,
    ...
    if (m_isToR) {
        // std::cout << "交换机端口:" << inDev << " 接收到信息" << std::endl;
        m_allInBytes[inDev] += packet->GetSize();
    }
...
void SwitchNode::SendToDev(Ptr<Packet> p, CustomHeader &ch) {
    ...
    // 接收pfc告警
    PfcNotify pfcNotify;
    if (p->PeekPacketTag(pfcNotify)) {
        Ipv4Address ipv4Address = Ipv4Address(ch.sip);
        // std::cout << "交换机接收到预警包 " << GetId() << " " << Settings::ip_to_node_id(ipv4Address) << std::endl;
        m_mmu->m_pfcWaring.UpdateWarning(p);
        return;
    }
    // Conga
    if (Settings::lb_mode == 3) {
        m_mmu->m_congaRouting.RouteInput(p, ch);
        // m_mmu->m_congaRouting.RouteInputPfc(p, ch, m_mmu->m_pfcWaring);
        return;
    }
...
void SwitchNode::DoSwitchSend(Ptr<Packet> p, CustomHeader &ch, uint32_t outDev, uint32_t qIndex) {
    ...
    if (qIndex != 0) {  // not highest priority
        ...
        if (!m_isToR && m_mmu->GetWarnMessage(inDev, qIndex)) {
            // 发送pfc预警
            // std::cout << "交换机pfc预警 " << GetId() << " 来源为:" << Settings::IdPortToId[m_mmu->m_gemmaRouting.GetSwitchPortKey(GetId(), inDev)] << std::endl;
            m_mmu->m_pfcWaring.SendPfcWarning(p,inDev,1);
        }
    }
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p) {
    ...
    if (qIndex != 0) {
        ...
        if (inDev != Settings::CONWEAVE_CTRL_DUMMY_INDEV && inDev != Settings::Gemma_CTRL_DUMMY_INDEV) {
            ...
            if (m_mmu->GetResumeMessage(inDev, qIndex)) {
                // pfc预警发生
                // std::cout << "pfc预警解除 " << GetId() << std::endl;
                m_mmu->m_pfcWaring.SendPfcWarning(p, inDev, 0);
            }
        }
    }
...
uint64_t SwitchNode::GetAllBytesInDev(uint32_t indev) {
    assert(indev < pCnt);
    return m_allInBytes[indev];
}
